...
import {SUBSCRIBER_API_URL} from "../app/constants";
...
export const fetchSubscribers = createAsyncThunk("subscriber/fetchSubscribers", async () => {
  const response = await fetch(`${SUBSCRIBER_API_URL}/all`);
...
export const deleteSubscriber = createAsyncThunk("subscribers/delete",async (id) => {
      await fetch(`${SUBSCRIBER_API_URL}/${id}`, {
    method: "DELETE",
  });
    return id;
  });
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchSubscribers.fulfilled, (state, action) => {
        state.loading = false;
        state.subscribers = action.payload;  // Store approved blogs here
      })
